...
def _read_learnings_manifest():
    """Read learnings manifest text (next fixes summary)."""
    learnings_script = Path(".forja-tools") / "forja_learnings.py"
...
def _read_event_stream():
    """Read .forja/event-stream.jsonl (unified event stream)."""
    events = []
    p = Path(".forja") / "event-stream.jsonl"
    if not p.exists():
        return events
    try:
        for line in p.read_text(encoding="utf-8").splitlines():
            ...
            try:
                ...
                print(f"  malformed JSONL in event-stream.jsonl: {exc}", file=sys.stderr)
    except OSError as exc:
        print(f"  could not read event-stream.jsonl: {exc}", file=sys.stderr)
    return events
...
def _compute_metrics(teammates, spec_review, plan_transcript,
    ...
    return {
        ...
        "event_stream": (event_stream or [])[:500],
    }
...
def cmd_report():
    ...
    event_stream = _read_event_stream()
