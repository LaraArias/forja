...
Usage:
    ...
    python3 .forja-tools/forja_context.py set <key> <value> --author <name> [--tags <t1,t2>] [--type DECISION|FACT|ASSUMPTION|OBSERVATION]
    ...
    python3 .forja-tools/forja_context.py audit [--type DECISION|FACT|ASSUMPTION|OBSERVATION]

Storage: context/store/*.json
Event stream: .forja/event-stream.jsonl
"""
...
LOCK_TIMEOUT = 5
LOCK_RETRIES = 3
VALID_ENTRY_TYPES = ("DECISION", "FACT", "ASSUMPTION", "OBSERVATION")
...
def _read_event_stream():
    """Read .forja/event-stream.jsonl."""
    if not EVENT_STREAM.exists():
        return []
    events = []
    try:
        for line in EVENT_STREAM.read_text(encoding="utf-8").splitlines():
            line = line.strip()
            if not line:
                continue
            try:
...
def cmd_set(args):
    ...
    while i < len(args):
        if args[i] == "--author" and i + 1 < len(args):
            ...
            entry_type = args[i + 1].upper()
            if entry_type not in VALID_ENTRY_TYPES:
                print(f"ERROR: Invalid type '{entry_type}'. Use: DECISION, FACT, ASSUMPTION, OBSERVATION")
                sys.exit(1)
            i += 2
        else:
            i += 1
    ...
    with StoreLock():
        ...
        # Preserve or set entry type
        if entry_type:
            data["type"] = entry_type
        elif existing and existing.get("type"):
    ...
    type_info = f" [{entry_type}]" if entry_type else ""
    print(f"{key}: {action} (v{version} by {author}){type_info}")

    # Emit to unified event stream
    event_data = {"key": key, "value": value, "author": author, "version": version}
    if tags:
        event_data["tags"] = tags
    _emit_event("context.set", event_data, agent=author)

    # Emit decision-specific event
    if entry_type:
        _emit_event("decision.logged", {
            "key": key, "value": value, "decision_type": entry_type, "author": author,
        }, agent=author)
...
def cmd_list(args):
    ...
    for var in matches:
        ...
        type_tag = f" [{var['type']}]" if var.get("type") else ""
        print(f"  {var['key']}: {val_str}{type_tag}")
...
def cmd_search(args):
    ...
    for var in matches:
        val_str = _compact_value(var["value"])
        type_tag = f" [{var['type']}]" if var.get("type") else ""
        print(f"  {var['key']}: {val_str}{type_tag}  (by {var.get('author', '?')})")
...
def cmd_audit(args):
    """Show decision timeline from context store and event stream."""
    # 1. Read all context vars with type field
    variables = _all_vars()
    typed = [v for v in variables if v.get("type")]

    # 2. Read event stream for decision events
    events = _read_event_stream()
    decision_events = [e for e in events if e.get("type", "").startswith("decision.")]

    # 3. Build timeline from typed context vars
    timeline = []
    for v in typed:
        timeline.append({
            "timestamp": v.get("updated_at", v.get("created_at", "")),
            "type": v.get("type", ""),
            "key": v["key"],
            "value": _compact_value(v["value"], max_len=60),
            "author": v.get("author", "?"),
        })

    # 4. Add decision events from event stream
    for e in decision_events:
        timeline.append({
            "timestamp": e.get("timestamp", ""),
            "type": e.get("data", {}).get("decision_type", "EVENT"),
            "key": e.get("data", {}).get("key", "?"),
            "value": _compact_value(e.get("data", {}).get("value", ""), max_len=60),
            "author": e.get("agent", "?"),
            "source": "event_stream",
        })
    ...
    # 5. Filter by type if --type flag provided
    filter_type = ""
    for i, arg in enumerate(args):
        if arg == "--type" and i + 1 < len(args):
            filter_type = args[i + 1].upper()
    if filter_type:
        timeline = [t for t in timeline if t["type"] == filter_type]

    # 6. Print timeline
    if not timeline:
        print("No decisions or typed entries found.")
        print("Use: forja_context.py set <key> <value> --author <name> --type DECISION")
        return

    print(f"Decision Audit ({len(timeline)} entries)\n")
    for t in timeline:
        ts = t["timestamp"][:19] if t["timestamp"] else "?"
        src = " (stream)" if t.get("source") == "event_stream" else ""
        print(f"  [{ts}] {t['type']:<12} {t['key']}: {t['value']}  (by {t['author']}){src}")
    print()
