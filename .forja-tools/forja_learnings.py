...
def cmd_apply():
    ...
    # ── 5. Anti-patterns → DOMAIN.md ──
    _ANTIPATTERN_KEYWORDS = re.compile(
        r"should not|don't|avoid|never|wrong|incorrect|anti.?pattern",
        re.IGNORECASE,
    )
    for entry in entries:
        cat = entry.get("category", "")
        ...
        if not learning:
            continue
        if cat in ("error-pattern", "kimi-finding") and _ANTIPATTERN_KEYWORDS.search(learning):
            domain_dir = Path("context/domains")
            if domain_dir.is_dir():
                for dmd in sorted(domain_dir.glob("*/DOMAIN.md")):
                    content = dmd.read_text(encoding="utf-8")
                    if "## Anti-patterns" not in content:
                        with open(dmd, "a", encoding="utf-8") as f:
                            f.write("\n## Anti-patterns\n\n")
                    line = f"- [LEARNED] {learning[:200]}"
                    if _append_to_file(dmd, line):
                        applied += 1

    # ── 6. Kimi findings → validation-rules.md ──
    for entry in entries:
        cat = entry.get("category", "")
        learning = entry.get("learning", "")
        if not learning:
            continue
        if cat == "kimi-finding":
            target = Path("context/company/validation-rules.md")
            _ensure_file(target, "# Validation Rules\n\nRules extracted from cross-model code review.\n")
            line = f"- [KIMI] {learning[:200]}"
            if _append_to_file(target, line):
                applied += 1
...
# ── Synthesize: JSONL → _learnings.md wisdom file ───────────────────

# Category → human-readable context description
_CATEGORY_CONTEXT = {
    "error-pattern": "Build failure",
    "kimi-finding": "Cross-model code review",
    "edge-case": "Edge case discovered",
    "architecture-decision": "Architecture decision",
    "spec-gap": "PRD/spec review",
    "assumption": "Unvalidated assumption",
    "unmet-requirement": "Outcome evaluation",
    "product-backlog": "Product decision needed",
}


def _extract_action(learning_text):
    """Extract the action part from a learning text."""
    for marker in ("Auto-fix:", "Action:"):
        if marker in learning_text:
            return learning_text.split(marker, 1)[1].strip()
    return ""


def _extract_short_title(learning_text):
    """Generate a short title from the learning text."""
    # Take first sentence or first 60 chars
    for sep in (". ", ": ", " — "):
        if sep in learning_text:
            title = learning_text.split(sep, 1)[0]
            if len(title) < 80:
                return title
    return learning_text[:60].rstrip(". ")


def cmd_synthesize():
    """Convert JSONL learnings into a human-readable _learnings.md wisdom file.

    Format per entry:
      ## [DATE] - Short Title
      **Context**: category description
      **Error**: source + learning text
      **Principle**: extracted principle
      **Action**: extracted action
    """
    entries = _read_all_entries()
    if not entries:
        print(f"{DIM}No learnings to synthesize{RESET}")
        return
    ...
    lines = [
        "# Forja Learnings — Accumulated Wisdom",
        "",
        "Principles extracted from build failures and reviews.",
        "Each entry documents what went wrong and how to prevent it.",
        "",
    ]
    ...
    for entry in entries:
        ...
        title = _extract_short_title(learning)
        context = _CATEGORY_CONTEXT.get(cat, cat)
        action = _extract_action(learning)

        if sev == "HIGH":
            # Full format for HIGH severity
            lines.append(f"## [{date_str}] - {title}")
            lines.append("")
            lines.append(f"**Context**: {context} (source: {source})")
            lines.append(f"**Error**: {learning}")
            lines.append(f"**Principle**: {title}")
            if action:
                lines.append(f"**Action**: {action}")
            lines.append("")
        else:
            # Compact format for MEDIUM/LOW
            lines.append(f"- **[{date_str}]** [{sev}] {learning[:150]}")

    # Write to _learnings.md
    wisdom_path = LEARNINGS_DIR / "_learnings.md"
    LEARNINGS_DIR.mkdir(parents=True, exist_ok=True)
    wisdom_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"{PASS_ICON} Synthesized {len(entries)} learnings into {wisdom_path}")
...
def main():
    if len(sys.argv) < 2:
        print(
            ...
            "  python3 .forja-tools/forja_learnings.py synthesize"
        )
        sys.exit(1)
    ...
    if command == "log":
        ...
        print("  Valid: log, manifest, extract, apply, synthesize")
        sys.exit(1)
